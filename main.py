...
def init_files():
    ...
    if not os.path.exists(PURCHASES_FILE):
        json.dump({}, open(PURCHASES_FILE, "w", encoding="utf-8"))
...
def add_referral(user_id, referrer_id):
    users = load_users()
    if str(referrer_id) in users and str(user_id) not in users.get(str(referrer_id), {}).get("referrals", []):
        users[str(referrer_id)]["referrals"].append(user_id)
        save_users(users)
...
def amount_keyboard(product_id, product_name, price):
    keyboard = InlineKeyboardMarkup(row_width=3)
    for i in range(1, 11):
        keyboard.add(InlineKeyboardButton(str(i), callback_data=f"amount_{product_id}_{i}_{price}"))
    
    btn_back = InlineKeyboardButton("◀️ Назад в каталог", callback_data="catalog")
    keyboard.add(btn_back)
    return keyboard

def payment_keyboard(amount):
    keyboard = InlineKeyboardMarkup(row_width=2)
    btn_crypto = InlineKeyboardButton("💎 Оплатить CryptoBot", callback_data=f"pay_crypto_{amount}")
    btn_back = InlineKeyboardButton("◀️ Назад", callback_data="balance")
    keyboard.add(btn_crypto, btn_back)
    return keyboard
...
def start_command(message):
    ...
    if len(message.text.split()) > 1:
        ...
        if str(referrer_id).isdigit() and int(referrer_id) != user_id:
            users = load_users()
            if str(referrer_id) in users and str(user_id) not in users[str(referrer_id)].get("referrals", []):
                add_referral(user_id, referrer_id)
    
    # Отправка приветствия с фото
    try:
        with open('welcome.jpg', 'rb') as photo:
            bot.send_photo(user_id, photo, caption=generate_profile_text(user_id, username), reply_markup=main_menu_keyboard())
    except:
        bot.send_message(user_id, generate_profile_text(user_id, username), reply_markup=main_menu_keyboard())
...
def callback_handler(call):
    ...
    if call.data == "back_to_menu":
        ...
        bot_username = bot.get_me().username
        ref_link = f"https://t.me/{bot_username}?start={user_id}"
        bot.answer_callback_query(call.id, f"Ссылка скопирована: {ref_link}", show_alert=True)
...
def process_amount(message, product_key, product):
    ...
    # Выдача товара
    # Здесь должна быть логика получения реальных токенов/данных
    # Сейчас это заглушка
...
def check_payment_callback(call):
    ...
    # Здесь должна быть проверка реального платежа
