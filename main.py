...
7️⃣ Продолжая использовать бота вы соглашаетесь со всеми правилами."""
    ...
    # ── Покупка ───────────────────────────────────────────────────────────────
        ...
        # Отправляем весь фиксированный набор текстов одним блоком
        total = len(items)
        for i, item_content in enumerate(items, 1):
            try:
                bot.send_message(
                    user_id,
                    f"📦 <b>{product['emoji']} {product['name']}</b> [{i}/{total}]\n"
                    f"━━━━━━━━━━━━━━━\n"
                ...
                try:
                    bot.send_message(user_id, f"[{i}/{total}]\n{item_content}")
                except:
                    pass
...
def handle_message(message):
    ...
    if state.get("awaiting_quantity"):
        ...
        stats = get_items_stats(product_key)
        if not product or quantity > stats["free"]:
            bot.send_message(user_id, f"❌ В наличии только {stats['free']} шт!")
            return
